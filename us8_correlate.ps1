$seg = Get-Content us8_segment.log
$errors = @()
...
foreach ($e in $errors) {
    ...
    if ($empIds.Count -eq 0) {
        # try to extract emp from the error text
        if ($e.err -match 'Empregado nao existe') { $note = 'Empregado lookup failed' } else { $note = '' }
    }
...
$out | Format-Table -AutoSize | Out-String | Out-File us8_correlation_report.txt -Encoding utf8
$summary = "Wrote us8_correlation_report.txt with $($out.Count) items"
